...
@Component({
  selector: 'desktop-add-marketplace',
  templateUrl: './desktop-add-marketplace.component.html',
  styleUrls: ['./desktop-add-marketplace.component.scss']
})
export class DesktopAddMarketplaceComponent implements OnInit {
  submitted = false;
  textRemainingName;
  textRemainingDesc;
  fd = new FormData();
  ...
  onSubmit(form: NgForm) {
    ...
    this.fd.set('seller', '7dbf7b8b-9cec-4dfb-9c5b-623a0a2feb22');
    ...
    for (const key in this.addProductForm.value) {
      if (key) {
        if (key === 'categoryArray') {
          ...
          this.fd.append('category', '[' + categories.toString() + ']');
        } else {
