...
@Component({
  ...
  styleUrls: ['./marketplace-categories.component.scss']
})
export class MarketplaceCategoriesComponent implements OnInit, AfterViewInit {
  @Input() data;
  ...
  constructor(public cdr: ChangeDetectorRef, public router: Router, public route: ActivatedRoute) { }
  ...
  routeTo(id, catName) {
    this.catData.emit({id: id, name: catName});
    if (!this.isSubcategory) {
      if (this.categoryType !== 'booking') {
        this.router.navigate([this.categoryType, id]);
      } else {
        this.router.navigate(['/', this.categoryType, 'search'], {queryParams: {cat: id}});
      }
    } else {
      console.log(this.categoryType, this.categoryId, id);
      if (this.categoryType !== 'booking') {
        this.router.navigate([this.categoryType, this.categoryId, id]);
      }
    }
  }
